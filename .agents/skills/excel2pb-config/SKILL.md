...
- When editing an existing sheet, preserve its column widths, colors, borders, and frozen panes. Keep row 4 descriptions horizontally and vertically centered, with wrapping enabled where needed.
...
6. Every data sheet has the correct four header rows; field and sheet names are valid; and data starts on row 5.
